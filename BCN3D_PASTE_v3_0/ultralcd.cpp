...
{
    setTargetHotend0(LAYWOOD_PREHEAT_HOTEND_TEMP);
    setTargetHotend1(LAYWOOD_PREHEAT_HOTEND_TEMP);
    setTargetHotend2(LAYWOOD_PREHEAT_HOTEND_TEMP);
    setTargetBed(LAYWOOD_PREHEAT_HPB_TEMP);
    fanSpeed = LAYWOOD_PREHEAT_FAN_SPEED;
    lcd_return_to_status();
    setWatch(); // heater sanity check timer
}
...
static void lcd_unload()
{
  ...
  while (!LCD_CLICKED)
    {
      encoderPosition = 0; 
      plan_buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], current_position[E_AXIS]-10.0, 3, active_extruder); 
    } 
    
        lcd_quick_feedback();
        currentMenu = lcd_change_menu;
...
static void lcd_change_menu()
{
    ...
    MENU_ITEM(submenu, MSG_UNLOAD, lcd_unload);
    MENU_ITEM(submenu, MSG_LOAD, lcd_load);
    END_MENU();
}
